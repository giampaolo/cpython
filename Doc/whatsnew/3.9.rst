...
The :class:`mmap.mmap` class now has an :meth:`~mmap.mmap.mlock` method to
access the ``mlock()`` system call, preventing that memory from being paged to
the swap area. If dealing with a file, subsequent reads from the file will be
satisfied from the cache, and not block on disk I/O (page fault).
(Contributed by Giampaolo Rodola in :issue:`XXX`.)
